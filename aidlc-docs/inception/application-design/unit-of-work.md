...
| `unit-kebbi-contract` | Kebbi Contract | `clients/kebbi/` + sibling repo | sibling repo APK | HTTP 契約、`nakanaori-kebbi` の `NakanaoriApi.kt` |
...
- **実装**: sibling repo `nakanaori-kebbi` — `NakanaoriApi.kt`、TTS/ASR
...
参照リポジトリ（VRM パターン）: [AIxR-CharaTomo-Web](https://github.com/SystemFriend/AIxR-CharaTomo-Web)（`CHARATOMO_MODELS` または sibling `../AIxR-CharaTomo-Web`）
...
Kebbi Android コードはこの repo **外** — sibling `nakanaori-kebbi`（`$NAKANAORI_KEBBI_ROOT`）
