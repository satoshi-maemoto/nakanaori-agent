...
| 実行時 API キー | **GCP Secret Manager** | `GEMINI_API_KEY`, `GOOGLE_TTS_CREDENTIALS_JSON` → Cloud Run `--set-secrets` |
| ローカル開発 | **`.env`**（gitignore） | `.env.example` をコピー |

`GEMINI_API_KEY` と TTS 認証 JSON は GitHub Secret に **置かない**。
...
### GOOGLE_TTS_CREDENTIALS_JSON — 審査用 ON / 普段 OFF

| 操作 | コマンド |
|------|----------|
| **ON**（TTS 有効） | Secret Manager に JSON 有効化 → `gcloud run services update nakanaori-api --region asia-northeast1 --set-secrets GOOGLE_TTS_CREDENTIALS_JSON=GOOGLE_TTS_CREDENTIALS_JSON:latest` |
| **OFF**（503 フォールバック） | `gcloud run services update nakanaori-api --region asia-northeast1 --remove-secrets GOOGLE_TTS_CREDENTIALS_JSON` |

Secret 登録例（サービスアカウント JSON を 1 行化）:
...
未設定時は `POST /v1/tts/synthesize` が **503** を返す（Web は音声なしで続行）。詳細: [google-cloud-tts-setup.md](./google-cloud-tts-setup.md)
...
| TTS 502 / 認証エラー | `GOOGLE_TTS_CREDENTIALS_JSON` secret 注入を確認；未設定なら 503 は正常 |
